...
debug("drawing smooth line")
	...
	if(obj.ld){
		...
		canto(canvasid).beginPath().moveTo(obj.lsmX*redrawMultiplier,obj.lsmY*redrawMultiplier).lineTo(obj.smX*redrawMultiplier,obj.smY*redrawMultiplier).stroke({lineWidth: lineW+(canvsFactor/obj.cf), lineCap:"none", strokeStyle: "rgba("+ obj.bc.r+", "+ obj.bc.g+", "+ obj.bc.b+", "+ brushAlpha+")"}).endPath();
	}else{
	//if (false) {
	if (obj.vX*obj.lvX + obj.vY*obj.lvY < 0 || obj.lvc) {	
		//quick change of velocity, draw a straight line
		canto(canvasid).beginPath().moveTo(obj.lsmX*redrawMultiplier,obj.lsmY*redrawMultiplier).lineTo(obj.smX*redrawMultiplier, obj.smY*redrawMultiplier).stroke({lineWidth: lineW+(canvsFactor/obj.cf), lineCap:"round", strokeStyle: "rgba("+ obj.bc.r+", "+ obj.bc.g+", "+ obj.bc.b+", "+ brushAlpha+")"}).endPath();
		
		canto(canvasid).beginPath().moveTo(obj.psX*redrawMultiplier,obj.psY*redrawMultiplier).lineTo(obj.lsmX*redrawMultiplier, obj.lsmY*redrawMultiplier).stroke({lineWidth: lineW+(canvsFactor/obj.cf), lineCap:"round", strokeStyle: "rgba("+ obj.bc.r+", "+ obj.bc.g+", "+ obj.bc.b+", "+ brushAlpha+")"}).endPath();
